...
def bulk_generate():
    ...
    Invokes Lambdas in the background and saves results to DB.
    """
    data = request.get_json()
    access_key = data.get('access_key', '').strip()
    secret_key = data.get('secret_key', '').strip()
    ...
    def background_process(app, job_id, users, access_key, secret_key, region):
        ...
        - Splits users into batches of 10
        - Distributes batches across all AWS regions (geos)
        - Processes batches per-geo in parallel, and within each geo in parallel
        """
        logger.info("[BULK] ========== BACKGROUND PROCESS STARTED ==========")
        logger.info(f"[BULK] Job ID: {job_id}")
        logger.info(f"[BULK] Total users: {len(users)}")
        logger.info(f"[BULK] Base region (from UI): {region}")

        # Ensure job exists
        with jobs_lock:
            if job_id not in active_jobs:
                logger.error(f"[BULK] Job {job_id} not found in active_jobs at start of background_process! Recreating.")
                active_jobs[job_id] = {
                    'total': len(users),
                    'completed': 0,
                    'success': 0,
        ...
        try:
            with app.app_context():
                logger.info("[BULK] App context entered successfully")

                import math

                USERS_PER_FUNCTION = 10  # Each Lambda invocation handles at most 10 users
                total_users = len(users)
                num_functions = math.ceil(total_users / USERS_PER_FUNCTION)

                logger.info("=" * 60)
                logger.info("[BULK] Function Calculation")
                ...
                # Assign each "function" a geo in round-robin
                functions_per_geo = {}  # {geo: [function_number]}
                for func_num in range(num_functions):
                    geo_index = func_num % len(AVAILABLE_GEO_REGIONS)
                    geo = AVAILABLE_GEO_REGIONS[geo_index]
                    functions_per_geo.setdefault(geo, []).append(func_num + 1)

                logger.info("=" * 60)
                logger.info("[BULK] Function Distribution Across Geos")
                logger.info(f"[BULK] Total functions: {num_functions}")
                logger.info(f"[BULK] Available geos: {len(AVAILABLE_GEO_REGIONS)}")
                for geo, fn_numbers in sorted(functions_per_geo.items()):
                    logger.info(f"[BULK]   - {geo}: {len(fn_numbers)} function(s) {fn_numbers}")
                logger.info("=" * 60)

                # Build user batches: each batch assigned to a function + geo
                user_batches = []  # List[(function_number, geo, [users])]
                logger.info(f"[BULK] Creating batches: total_users={total_users}, num_functions={num_functions}")
                for func_num in range(num_functions):
                    ...
                    logger.info(
                        f"[BULK] Function {func_num + 1} ({geo}) will process "
                        f"{len(batch_users)} user(s): {[u['email'] for u in batch_users[:3]]}"
                        f"{'...' if len(batch_users) > 3 else ''}"
                    )

                # Group batches by geo
                batches_by_geo = {}  # {geo: [(func_num, batch_users)]}
                for func_num, geo, batch_users in user_batches:
                    batches_by_geo.setdefault(geo, []).append((func_num, batch_users))

                logger.info("=" * 60)
                logger.info("[BULK] Batches per geo:")
                for geo, geo_batches in sorted(batches_by_geo.items()):
                    total_geo_users = sum(len(batch) for _, batch in geo_batches)
                    logger.info(f"[BULK]   - {geo}: {len(geo_batches)} batch(es), {total_geo_users} user(s)")
                logger.info(f"[BULK] TOTAL GEOS TO PROCESS: {len(batches_by_geo)}")
                logger.info("=" * 60)

                USERS_PER_BATCH = 10  # hard limit enforced at Lambda level

                def process_user_batch_sync(user_batch, lambda_function_name, lambda_region):
                    """
                    Process up to 10 users synchronously with a single Lambda invocation.
                    """
                    with app.app_context():
                        MAX_USERS_PER_BATCH = 10
                        if len(user_batch) > MAX_USERS_PER_BATCH:
                            logger.error(
                                f"[BULK] [{lambda_function_name}] CRITICAL: "
                                f"batch has {len(user_batch)} users, truncating to {MAX_USERS_PER_BATCH}"
                            )
                        ...
                        lam_batch = session_batch.client(
                            "lambda",
                            config=Config(
                        ...
                        dynamodb_batch = session_batch.resource(
                            "dynamodb",
                        ...
                        batch_results = []
                        users_to_process = user_batch

                        logger.info(
                            f"[BULK] [{lambda_function_name}] Will process "
                            f"{len(users_to_process)} user(s) in {lambda_region}"
                        )

                        # mark emails as processing (duplicate prevention)
                        for u in users_to_process:
                            email = u["email"]
                            with processing_lock:
                                if email in processing_emails:
                                    logger.warning(
                                        f"[BULK] WARNING: {email} already being processed in another batch"
                                    )
                                processing_emails.add(email)

                        payload = {
                            "users": [{"email": u["email"], "password": u["password"]} for u in users_to_process]
                        }
                        ...
                        try:
                            ...
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    raw_body = resp.get("Payload").read().decode("utf-8")
                                    logger.info(
                                        f"[BULK] [{lambda_function_name}] Lambda status={resp.get('StatusCode')}"
                                    )

                                    try:
                                        lambda_response = json.loads(raw_body)
                                    except json.JSONDecodeError as je:
                                        logger.error(
                                            f"[BULK] [{lambda_function_name}] Invalid JSON from Lambda: {je}"
                                        )
                                        for u in users_to_process:
                                            batch_results.append(
                                                {
                                                    "email": u["email"],
                                                    "success": False,
                                                    "error": f"Invalid JSON response: {raw_body[:200]}",
                                                }
                                            )
                                        return batch_results

                                    # New batch format: {"status": "completed", "results": [...]}
                                    if (
                                        lambda_response.get("status") == "completed"
                                        and "results" in lambda_response
                                    ):
                                        lambda_results = lambda_response["results"]
                                        logger.info(
                                            f"[BULK] [{lambda_function_name}] "
                                            f"Lambda returned {len(lambda_results)} results"
                                        )
                                        for r in lambda_results:
                                            ...
                                            if status == "success" and app_password:
                                                try:
                                                    save_app_password(email, app_password)
                                                    logger.info(f"[BULK] Saved app password for {email}")
                                                except Exception as db_err:
                                                    logger.error(
                                                        f"[BULK] Failed to save password for {email}: {db_err}"
                                                    )
                                                batch_results.append(
                                                    {
                                                        "email": email,
                                    ...
                                    if len(users_to_process) == 1:
                                        u = users_to_process[0]
                                        status = lambda_response.get("status", "unknown")
                                        app_password = lambda_response.get("app_password")
                                        error_msg = lambda_response.get(
                                        ...
                                        if status == "success" and app_password:
                                            try:
                                                ...
                                                logger.error(
                                                    f"[BULK] Failed to save to DB for {u['email']}: {db_err}"
                                                )
                                            batch_results.append(
                                                {
                                                    "email": u["email"],
                                    ...
                                    # If we reach here and have multiple users, response format is wrong
                                    logger.error(
                                        f"[BULK] [{lambda_function_name}] Invalid response format from Lambda"
                                    )
                                    for u in users_to_process:
                                        batch_results.append(
                                            {
                                    ...
                                    if code in ("TooManyRequestsException", "ThrottlingException"):
                                        ...
                                        logger.error(
                                            f"[BULK] [{lambda_function_name}] AWS error {code}: {msg}"
                                        )
                                        for u in users_to_process:
                                            batch_results.append(
                                                {
                                                    "email": u["email"],
                                                    "success": False,
                                                    "error": f"AWS Error ({code}): {msg}",
                                                }
                                            )
                                        return batch_results
                                except Exception as invoke_err:
                                    if "timeout" in str(invoke_err).lower():
                                        logger.error(
                                            f"[BULK] [{lambda_function_name}] Read timeout on attempt {attempt+1}"
                                        )
                                        if attempt == max_retries - 1:
                                            for u in users_to_process:
                                                batch_results.append(
                                                    {
                                                        "email": u["email"],
                                                        "success": False,
                                                        "error": "Read timeout from Lambda",
                                                    }
                                                )
                                            return batch_results
                                        time.sleep(5)
                                        ...
                                        logger.error(
                                            f"[BULK] [{lambda_function_name}] Invocation error: {invoke_err}"
                                        )
                                        if attempt == max_retries - 1:
                                            for u in users_to_process:
                                                batch_results.append(
                                                    {
                                                        "email": u["email"],
                                                        "success": False,
                                                        "error": f"Invocation error: {invoke_err}",
                                                    }
                                                )
                                            return batch_results
                                        time.sleep(2)
                ...
                def process_geo_parallel(geo, geo_batches_list):
                    """
                    For a single geo (region):
                    - Verify credentials
                    - For each batch, invoke the shared Lambda function in that region
                    - All batches in that geo processed in parallel
                    """
                    geo_results = []

                    try:
                        logger.info("=" * 60)
                        logger.info(f"[BULK] [{geo}] START PARALLEL PROCESSING")
                        logger.info(f"[BULK] [{geo}] Total batches: {len(geo_batches_list)}")

                        # basic session sanity check
                        session_boto_geo = boto3.Session(
                            aws_access_key_id=access_key,
                            aws_secret_access_key=secret_key,
                            region_name=geo,
                        )
                        sts = session_boto_geo.client("sts")
                        identity = sts.get_caller_identity()
                        logger.info(
                            f"[BULK] [{geo}] Credentials OK, account={identity.get('Account')}"
                        )

                        # We'll use the same function name in every region;
                        # region is handled via client config.
                        ...
                        logger.info(
                            f"[BULK] [{geo}] Using {max_workers} worker thread(s) for this geo"
                        )

                        def run_single_batch(func_num, batch_users, batch_index):
                            """Wrapper for one batch in this geo."""
                            logger.info(
                                f"[BULK] [{geo}] Batch #{batch_index+1}/{len(geo_batches_list)} "
                                f"func_num={func_num}, users={len(batch_users)}"
                            )
                            batch_res = process_user_batch_sync(
                                batch_users, lambda_function_name, lambda_region=geo
                            )
                            # Update job stats
                            with jobs_lock:
                        ...
                        with ThreadPoolExecutor(max_workers=max_workers) as pool:
                            future_map = {}
                            for idx, (func_num, batch_users) in enumerate(geo_batches_list):
                                fut = pool.submit(run_single_batch, func_num, batch_users, idx)
                                future_map[fut] = (func_num, idx)

                            for fut in as_completed(future_map):
                                func_num, idx = future_map[fut]
                                try:
                                    res = fut.result()
                                    geo_results.extend(res)
                                    ok = sum(1 for r in res if r.get("success"))
                                    logger.info(
                                        f"[BULK] [{geo}] Batch func={func_num} finished "
                                        f"({ok}/{len(res)} success)"
                                    )
                                except Exception as e:
                                    logger.error(
                                        f"[BULK] [{geo}] Exception in batch func={func_num}: {e}"
                                    )
                                    logger.error(traceback.format_exc())
                                    # mark all users in that batch as failed
                                    _, batch_users = geo_batches_list[idx]
                                    for u in batch_users:
                                        geo_results.append(
                                            {
                                                "email": u["email"],
                                                "success": False,
                                                "error": f"Batch exception in {geo}: {e}",
                                            }
                                        )
                        ...
                        logger.info("=" * 60)
                        logger.info(
                            f"[BULK] [{geo}] GEO DONE: {success_count}/{len(geo_results)} success, {fail_count} failed"
                        )
                        logger.info("=" * 60)
                        return geo_results
                        ...
                        logger.error("=" * 60)
                        logger.error(
                            f"[BULK] [{geo}] CRITICAL ERROR in process_geo_parallel: {geo_err}"
                        )
                        logger.error(traceback.format_exc())
                        logger.error("=" * 60)
                        # mark all users in this geo as failed
                        failed_results = []
                                ...
                                failed_results.append(
                                    {
                                        "email": u["email"],
                                        "success": False,
                                        "error": f"Geo processing failed for {geo}: {geo_err}",
                                    }
                                )
                ...
                logger.info("=" * 60)
                logger.info("[BULK] STARTING PARALLEL GEO PROCESSING")
                logger.info(f"[BULK] Total geos: {len(batches_by_geo)}")
                logger.info(f"[BULK] Geos: {list(batches_by_geo.keys())}")
                logger.info("=" * 60)

                all_geo_results = []
                max_geo_workers = max(1, len(batches_by_geo))

                with ThreadPoolExecutor(max_workers=max_geo_workers) as geo_pool:
                    future_map = {}
                    for geo, geo_batches in batches_by_geo.items():
                        future = geo_pool.submit(process_geo_parallel, geo, geo_batches)
                        future_map[future] = geo

                    for future in as_completed(future_map):
                        geo = future_map[future]
                        try:
                            geo_results = future.result(timeout=3600)
                            all_geo_results.extend(geo_results)
                            logger.info(
                                f"[BULK] [{geo}] GEO future completed with {len(geo_results)} results"
                            )
                        except Exception as e:
                            logger.error(
                                f"[BULK] [{geo}] GEO future EXCEPTION: {e}"
                            )
                            logger.error(traceback.format_exc())
                            # mark all users for this geo as failed
                                        ...
                                        all_geo_results.append(
                                            {
                                                "email": u["email"],
                                                "success": False,
                                                "error": f"Geo future exception for {geo}: {e}",
                                            }
                                        )
                ...
                with jobs_lock:
                    if job_id in active_jobs:
                        ...
                        logger.info(
                            f"[BULK] Job {job_id} completed: "
                            f"{active_jobs[job_id]['completed']}/{active_jobs[job_id]['total']} processed, "
                            f"{active_jobs[job_id]['success']} success, "
                            f"{active_jobs[job_id]['failed']} failed"
                        )
                    else:
                        # fallback (should not happen)
                        active_jobs[job_id] = {
                            "total": len(users),
                            "completed": len(all_geo_results),
                            "success": sum(1 for r in all_geo_results if r.get("success")),
                            "failed": sum(1 for r in all_geo_results if not r.get("success")),
    ...
    threading.Thread(
        ...
        args=(app, job_id, users, access_key, secret_key, region),
        daemon=True,
    ).start()
    ...
    return jsonify(
        {
            "success": True,
            "job_id": job_id,
            "message": f"Started processing {len(users)} users",
        }
    )
